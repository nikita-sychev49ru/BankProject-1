...
def filter_by_currency(transactions_list: List, currency: str) -> Any:
    ...
    elif not any(transaction.get('operationAmount', {}).get('currency', {}).get('code') == currency for transaction in
                 transactions_list):
...
def transaction_descriptions(transactions_list: List[Dict]) -> Generator[str | None | Any, Any, None]:
    ...
    if not transactions_list:
        ...
        yield from (transaction.get('description', 'Отсутствует описание транзакции') for transaction in
                    transactions_list)
