...
@patch('requests.get')
def test_get_amount_transaction1(mock_get_amount):
    """Тест для функции конвертации суммы в рублях - успешная отработка"""

    mock_get_amount.return_value.json.return_value = {'success': True,
                                                      'query': {'from': 'USD', 'to': 'RUB', 'amount': 8221.37},
                                                      'info': {'timestamp': 1748135764, 'rate': 79.342042},
                                                      'date': '2025-05-25', 'result': 652300.283838}
    transaction = {
        "id": 41428829,
        "state": "EXECUTED",
    ...
    mock_get_amount.assert_called_once_with(
        'https://api.apilayer.com/exchangerates_data/convert?to=RUB&from=USD&amount=8221.37&apikey=ET8qjUQNBxdJKOFnDW8g7f1hSk1QYcFu')
